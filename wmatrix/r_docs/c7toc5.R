...
slice_max(mean_freq, n=n) %>%
  ...
  name <- sym(glue::glue("top_{n}_{type}gram_{cut}"))
  l <- list(df)
...
beta <- C7_tops <- pmap(vals, ~get_top_n(data = C7_dt, n = {..2}, cut = {..3}, type = {..1})) %>% 
## flatten the list by one level
flatten %>% 
## enframe to created a nested tibble
enframe(name = "setting", value = "data")

## create a columns of all the selected posgrams as strings
beta %<>% mutate(strings = map(data, ~.x %>% pull(pos) %>% unique))
...
C7_df <- bind_rows(C7_tri,C7_bi,C7_uni)


# match the pos strings for each setting to subset the C7_ngrams df
beta %<>% mutate(for_pivot = map(.x= strings, .f = ~C7_df%>% filter(pos %in% .x)))
